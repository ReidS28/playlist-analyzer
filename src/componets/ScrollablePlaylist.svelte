	...
	<div class="flex flex-col p-2 gap-x3 h-80 w-full shrink-0 bg-red-500-">
		<div class="flex flex-row gap-1 h-16/20 w-full">
			...
			<div class="flex flex-col w-full bg-teal-500-">
				<span class={playlistTitleStyle}>Text 1</span>
				<span class={playlistTitleStyle}>Text 2</span>
				<span class={playlistTitleStyle}>Text 3</span>
			</div>
		</div>
		{#if data?.name}
			<div class="h-full text-5xl primaryText w-full bg-orange-500-">{data.name}</div>
		{/if}
	</div>
	<hr class="w-9/10" />
	<ul
